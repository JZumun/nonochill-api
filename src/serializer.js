...
const serialize = ({ width, height, colors, column, row, colorScheme = [] }) => {
  return btoa(
    [
      width,
      height,
      colors,
      serializeRuleGroup(column),
      serializeRuleGroup(row),
      colorScheme.join("?"),
    ].join("//")
  );
};
...
const deserializeList = string =>
  string
    .split("}{")
    .map(deserializeRule)
    .filter(({ count, val }) => count && val);
